...
These wheels are loaded at runtime by `core/pyodide-host.ts`. They are committed to the repo as build artifacts so that the project is reproducible offline and isn't hostage to upstream PyPI / npm uptime.
...
```
cd pyodide-build/cytriangle
./build.sh         # see the script for details
cp /tmp/cytriangle/dist/cytriangle-*.whl ../../wheels/
```

Verify the resulting sha256 matches what `npm test` consumes; update this README when versions change.
